...
module.exports.findOrCreate = function(url, callback) {
    ...
    URL.findOne({url: url}, function(err, result) {
        if(result) {
            ...
            // create new
            next += Math.ceil(Math.random() * 5);
            URL.create({key: smart.num2str(next), url: url}, callback);
        }
    });
};
